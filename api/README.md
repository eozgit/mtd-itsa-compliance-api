...
*   **API Specification**: OpenAPI 3.0 (generated by Swashbuckle.AspNetCore)
...
*   **OpenAPI Specification Generation**: Provides a machine-readable OpenAPI (Swagger) JSON specification for the entire API, accessible at `/swagger/v1/swagger.json`.
...
*   **OpenAPI Specification (via Swashbuckle.AspNetCore)**:
    *   The project uses `Swashbuckle.AspNetCore` (configured in `Program.cs` via `AddEndpointsApiExplorer()` and `AddSwaggerGen()`) to automatically generate the OpenAPI 3.0 JSON specification from the Minimal API endpoints.
    *   This provides a discoverable and machine-readable contract for client applications or other tooling. The specification includes details for JWT Bearer token authentication.
...
### Accessing the OpenAPI Specification (JSON)

While the Swagger UI is not enabled, the raw OpenAPI JSON specification is available at:

```
http://localhost:5129/swagger/v1/swagger.json
```

You can retrieve it using `curl`:
```bash
curl http://localhost:5129/swagger/v1/swagger.json
```
...
*   **Implement Real JWT Authentication**: Replace the mock JWT with a production-ready JWT implementation (e.g., using `Microsoft.AspNetCore.Authentication.JwtBearer` with proper key management and token validation).
