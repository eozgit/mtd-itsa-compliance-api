...
public class BusinessAndQuarterIntegrationTests : IClassFixture<CustomWebApplicationFactory<Program>>
{
    ...
    public async Task GetQuarters_ReturnsQuartersWithFinancialSummaries()
    {
        ...
        // Manually create mock quarters with varying income/expenses and statuses
        var mockQuarters = new List<QuarterlyUpdate>
        {
            // Submitted quarter with profit
            new QuarterlyUpdate { Id = ObjectId.GenerateNewId().ToString(), BusinessId = businessId, TaxYear = "2025/26", QuarterName = "Q1", Status = "SUBMITTED", TaxableIncome = 15000.00m, AllowableExpenses = 3000.00m, NetProfit = 12000.00m, SubmissionDetails = new SubmissionDetails{ RefNumber = "REF1", SubmittedAt = DateTime.UtcNow.AddDays(-30) } },
            // Another submitted quarter with profit
            new QuarterlyUpdate { Id = ObjectId.GenerateNewId().ToString(), BusinessId = businessId, TaxYear = "2025/26", QuarterName = "Q2", Status = "SUBMITTED", TaxableIncome = 20000.00m, AllowableExpenses = 4000.00m, NetProfit = 16000.00m, SubmissionDetails = new SubmissionDetails{ RefNumber = "REF2", SubmittedAt = DateTime.UtcNow.AddDays(-15) } },
            // Draft quarter with data (should not count towards submitted totals)
            new QuarterlyUpdate { Id = ObjectId.GenerateNewId().ToString(), BusinessId = businessId, TaxYear = "2025/26", QuarterName = "Q3", Status = "DRAFT", TaxableIncome = 5000.00m, AllowableExpenses = 1000.00m, NetProfit = 4000.00m },
            // Draft quarter with zero data (to show variety)
        ...
        // Verify individual quarters have correct data and calculated NetProfit (API calculates NetProfit)
        // Ordering by TaxYear and QuarterName to match the API's response ordering
        var orderedQuarters = quartersResponse.Quarters.OrderBy(q => q.TaxYear).ThenBy(q => q.QuarterName).ToList();

        Assert.Equal(mockQuarters[0].TaxableIncome, orderedQuarters[0].TaxableIncome);
        Assert.Equal(mockQuarters[0].AllowableExpenses, orderedQuarters[0].AllowableExpenses);
        Assert.Equal(mockQuarters[0].TaxableIncome - mockQuarters[0].AllowableExpenses, orderedQuarters[0].NetProfit);
        Assert.Equal(mockQuarters[0].Status, orderedQuarters[0].Status);

        Assert.Equal(mockQuarters[1].TaxableIncome, orderedQuarters[1].TaxableIncome);
        Assert.Equal(mockQuarters[1].AllowableExpenses, orderedQuarters[1].AllowableExpenses);
        Assert.Equal(mockQuarters[1].TaxableIncome - mockQuarters[1].AllowableExpenses, orderedQuarters[1].NetProfit);
        Assert.Equal(mockQuarters[1].Status, orderedQuarters[1].Status);

        Assert.Equal(mockQuarters[2].TaxableIncome, orderedQuarters[2].TaxableIncome);
        Assert.Equal(mockQuarters[2].AllowableExpenses, orderedQuarters[2].AllowableExpenses);
        Assert.Equal(mockQuarters[2].TaxableIncome - mockQuarters[2].AllowableExpenses, orderedQuarters[2].NetProfit);
        Assert.Equal(mockQuarters[2].Status, orderedQuarters[2].Status);

        // Verify cumulative financial summaries
        // Total Net Profit from SUBMITTED quarters: 12000.00 + 16000.00 = 28000.00
        decimal expectedTotalNetProfitSubmitted = 28000.00m;
        // Taxable amount: 28000.00 - 12570.00 (personal allowance) = 15430.00
        // Cumulative Estimated Tax Liability: 15430.00 * 0.20 (20% tax rate) = 3086.00
        decimal expectedCumulativeEstimatedTaxLiability = 3086.00m;
